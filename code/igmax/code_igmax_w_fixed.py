...
def add_exactly(s: Solver, lits: list[int], w: int, next_var: int) -> int:
    """
    For sum(lits) == w we use AtMost(w) and AtLeast(w).
    AtLeast(w) <=> sum(~lits) <= n-w
    """
    n = len(lits)
    next_var = add_atmost(s, lits, w, next_var)
    next_var = add_atmost(s, [-x for x in lits], n - w, next_var)
    return next_var
...
def find_polynomial_Q(P_coeffs, t, d, w):
    ...
    # the constraint for EXACT weight w: sum_k z_k == w
    next_var = add_exactly(solver, z_vars, w, next_var)
...
if __name__ == "__main__":
    t = 200  # Degree of P
    d = 70  # Degree of Q
    w = 10  # EXACT hamming weight for P*Q
